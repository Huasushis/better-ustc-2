...
impl CASClient {
    ...
    fn aes_encrypt(data: &str, key_base64: &str) -> Result<String> {
        let key = general_purpose::STANDARD.decode(key_base64).context("Invalid base64 key")?;
        if key.len() != 16 { bail!("Key length expected 16, got {}", key.len()); }
    ...
    fn get_usr_and_pwd(&self, username: Option<&str>, password: Option<&str>) -> Result<(String, String)> {
    ...
    pub async fn login_by_pwd(&self, username: Option<&str>, password: Option<&str>) -> Result<()> {
        ...
        let crypto = re_crypto.captures(&resp).and_then(|c| c.get(1)).map(|m| m.as_str()).context("Missing crypto")?.to_string();
        let flow_key = re_flow.captures(&resp).and_then(|c| c.get(1)).map(|m| m.as_str()).context("Missing flowkey")?.to_string();
        ...
        if !final_url.contains("cas/login") {
            ...
            let re_err = Regex::new(r#"<div\s+class="alert alert-danger"\s+id="login-error-msg">\s*<span>([^<]+)</span>\s*</div>"#)?;
            if let Some(caps) = re_err.captures(&text) {
                bail!("Login failed: {}", &caps[1]);
            }
            let debug_text = if text.len() > 200 { &text[..200] } else { &text };
            bail!("Login failed: Unknown error (status code {}). Body start: {}", status, debug_text);
        }
    }
    ...
    pub async fn get_info(&self) -> Result<UserInfo> {
        ...
        if !resp.status().is_success() {
             bail!("get_info failed. Status: {}. Maybe cookie expired?", resp.status());
        }
        ...
        let user_resp: Value = serde_json::from_str(&user_resp_text)
            .context("Failed to parse getUser JSON")?;
        
        let object_id = user_resp["objectId"].as_str().context("Failed to get object_id")?;
        let username = user_resp["username"].as_str().unwrap_or_default().to_string();
        ...
        let pid_resp: Value = serde_json::from_str(&pid_resp_text)
            .context("Failed to parse getPersonId JSON")?;
            
        let person_id = pid_resp["data"].as_str().context("Failed to get person_id")?;
        ...
        let info_resp: Value = serde_json::from_str(&info_resp_text)
            .context("Failed to parse userInfo JSON")?;
