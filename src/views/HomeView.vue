...
const loadAll = async () => {
  ...
  await Promise.all([activityStore.fetchRecommended(), activityStore.fetchAll()]).catch((e) =>
...
const filtered = (list: any[]) => {
  ...
  return list.filter((a) => {
    if (kw) {
      const txt = `${a.name} ${a.raw?.organizer_dictText || ''} ${a.raw?.placeInfo || ''}`.toLowerCase()
      if (!txt.includes(kw)) return false
    }
    if (org) {
      const o = `${a.raw?.organizer_dictText || a.raw?.businessDeptName || ''}`.toLowerCase()
      if (!o.includes(org)) return false
    }
    if (mods && mods.length) {
      const m = a.raw?.module
...
<template>
  <div class="min-h-screen">
    <NavBar title="首页" fixed />
    <div class="pt-12 px-3 pb-4 space-y-3">
      ...
      <PullRefresh v-model="refreshing" @refresh="loadAll">
